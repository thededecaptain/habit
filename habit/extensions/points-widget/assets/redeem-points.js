(function () {
  ...
  function cloneTemplate(id) {
    var t = document.getElementById(id || "habit-redeem-template");
  ...
  function initReferral(root, cart) {
    if (!root || root.getAttribute("data-habit-ready") === "1") return;
    root.setAttribute("data-habit-ready", "1");
  ...
  function injectReferral(btn, cart) {
  ...
  function ensure() {
    ...
    var hasRedeem = !!document.getElementById("habit-redeem-template");
    var hasReferral = !!document.getElementById("habit-referral-template");
    if (!hasRedeem && !hasReferral) return;
    cartJson()
      .then(function (cart) {
        checkoutButtons().forEach(function (btn) {
          if (hasRedeem) inject(btn, cart);
          if (hasReferral) injectReferral(btn, cart);
        });
      })
      .catch(function () {});
  }
    ...
    new MutationObserver(function (muts) {
      for (var i = 0; i < muts.length; i++) {
        ...
        for (var j = 0; j < nodes.length; j++) {
          ...
          if (n.getAttribute && (n.getAttribute("data-habit-redeem") != null || n.getAttribute("data-habit-referral") != null)) continue;
